...
class GRUCausalMMM(nn.Module):
    def __init__(self, A_prior, n_media=10, ctrl_dim=15, hidden=64, n_regions=2):
        ...
        self.w_raw = nn.Linear(hidden, n_media)          # → softplus ≥0
    ...
    def forward(self, Xm, Xc, R):                      # all seq [B,T,*]
        ...
        # Step 2: Apply Hill transformation directly to adstock output
        # The Hill transformation will handle the saturation effect
        hill_out = self.hill(adstock_out)  # [B,T,n_media]
        ...
        # Note: Removed week-wise normalization as it kills signal variation
        # Temporal smoothness penalty will handle week-0 spikes instead
        
        # Calculate actual contributions using raw adstock values (in original scale)
        # This preserves the true spend dynamics and scale for business interpretation
        media_contributions = adstock_out * w_pos  # [B,T,n_media] - contributions in original scale
        
        # But use transformed values for the final output calculation
        media_term = media_contributions.sum(-1)  # [B,T] - total media contribution
        ...
        return y_scaled, w_pos, media_contributions  # [B,T], [B,T,n_media], [B,T,n_media]
...
def prepare_data_for_training(df, params):
    ...
    for region in regions:
        region_data[region] = region_data[region].iloc[:min_length]
    
    # Create standardized feature matrices
    n_regions = len(regions)
    n_time_steps = min_length
